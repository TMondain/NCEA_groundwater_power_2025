...
fpower0 <- fpower_LR0 <- fpower_LR0_anova <-rep(NA,max(user_inputs$Scenario))



dat=read.csv("N://NCEA//GWDTE//NEW contract//water_quality_data_cleaned.csv")

modparam <- glmmTMB(gw0117_value~1+(1|year)+(1|sampling_point),data=dat,family=Gamma(link = "log"))


scenarios=read.csv("N://NCEA//GWDTE//NEW contract//Power//N_scenarios.csv")
...
for (ss in 1:12){
  ...
  ########################################################################
  nsim = 100 # select a number of simulations
