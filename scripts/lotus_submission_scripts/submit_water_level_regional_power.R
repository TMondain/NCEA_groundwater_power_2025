...
## read a dataset to get column names
dataset <- read.csv("/gws/nopw/j04/ceh_generic/thoval/ncea/groundwater_power/data/processed/water_level/dip_data_5_95quantile_summary.csv")

# read the scenarios
scenarios <- read.csv("/gws/nopw/j04/ceh_generic/thoval/ncea/groundwater_power/data/scenarios/WL_scenarios_reg.csv")
scenarios$X <- NULL
scenarios$days <- NULL

pars_init <- expand.grid(response_var = c("water_level", "perc_chg_ind", "cent_5_value", "cent_95_value"),
                         sample_column = grep("sampledtype_", colnames(dataset), value = TRUE),
                         site_column = "sampling_point",
                         nsim = 100,
                         samfreq = 1,
                         save_loc = "/gws/nopw/j04/ceh_generic/thoval/ncea/groundwater_power/data/simulations/power/water_level_regional",
                         stringsAsFactors = FALSE)

pars_full <- expand.grid.df(pars_init, scenarios)

datasets_full <- c("/gws/nopw/j04/ceh_generic/thoval/ncea/groundwater_power/data/processed/water_level/dip_data_mn_5yrperc_change.csv;/gws/nopw/j04/ceh_generic/thoval/ncea/groundwater_power/data/processed/water_level/site_data_mn_5yrperc_change.csv")

datasets_summarised <- c("/gws/nopw/j04/ceh_generic/thoval/ncea/groundwater_power/data/processed/water_level/dip_data_5_95quantile_summary.csv;/gws/nopw/j04/ceh_generic/thoval/ncea/groundwater_power/data/processed/water_level/site_data_5_95quantile_summary.csv")


pars <- pars_full %>% 
  mutate(datasets = ifelse(
    pars_full$response_var == "water_level" | pars_full$response_var == "perc_chg_ind",
    datasets_full,
    ifelse(
      pars_full$response_var == "cent_5_value" | pars_full$response_var == "cent_95_value",
      datasets_summarised,
      "TWIT"
    )
  ),
  model_pars = ifelse(
    pars_full$response_var == "water_level" | pars_full$response_var == "perc_chg_ind",
    paste(c("month", "year", "sampling_point"), collapse = ";"),
    ifelse(
      pars_full$response_var == "cent_5_value" | pars_full$response_var == "cent_95_value",
      paste(c("year", "sampling_point"), collapse = ";"),
      "TWIT"
    )
  ),
  random_effect = ifelse(
    pars_full$response_var == "water_level" | pars_full$response_var == "perc_chg_ind",
    paste(c("month", "sampling_point"), collapse = ";"),
    ifelse(
      pars_full$response_var == "cent_5_value" | pars_full$response_var == "cent_95_value",
      paste(c("sampling_point"), collapse = ";"),
      "TWIT"
    )
  ),
  yearly_samfreq_column = ifelse(
    pars_full$response_var == "water_level" | pars_full$response_var == "perc_chg_ind",
    "month",
    ifelse(
      pars_full$response_var == "cent_5_value" | pars_full$response_var == "cent_95_value",
  ...
  yearly_samfreq = ifelse(yearly_samfreq_column == "month", 12, 1),
  nosite.yr = nosite,
  effect.size = eff,
  data_proportions = prop_tele
  )
